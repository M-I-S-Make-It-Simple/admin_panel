...
// Додаємо GET метод для отримання конкретного клубу
export async function GET(
  ...
  try {
    ...
    console.log('🔍 GET запит для clubs studios з ID:', id);

    const clubsStudios = await prisma.clubsStudios.findUnique({
      where: { id },
    });
    ...
    const result = {
      ...clubsStudios,
      photoUrls: clubsStudios.photoUrls ? JSON.parse(clubsStudios.photoUrls) : []
    };
...
export async function PUT(
  ...
  try {
    ...
    console.log('🔄 PUT запит для clubs studios з ID:', id);
    ...
    const clubsStudios = await prisma.clubsStudios.update({
      where: { id },
      data
    });
    ...
    console.error('Error updating clubs studios:', error);
    
    const errorResponse = NextResponse.json(
      { error: 'Failed to update clubs studios' },
      { status: 500 }
    );
...
export async function DELETE(
  ...
  try {
    ...
    console.log('🗑️ DELETE запит для clubs studios з ID:', id);

    const clubsStudios = await prisma.clubsStudios.delete({
      where: { id },
    });
    ...
    console.error('Error deleting clubs studios:', error);
    
    const errorResponse = NextResponse.json(
      { error: 'Failed to delete clubs studios' },
      { status: 500 }
    );
