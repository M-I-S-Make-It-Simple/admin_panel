...
export async function PUT(
  ...
  try {
    ...
    const updatedItem = await prisma.patrioticEducation.update({
      where: { id },
      data: {
        heading: heading || '',
    ...
    console.error('Error updating patriotic-education:', error);
    return NextResponse.json(
      { error: 'Failed to update patriotic-education' },
      { status: 500 }
    );
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    const deletedItem = await prisma.patrioticEducation.delete({
      where: { id },
    });

    return NextResponse.json({ 
      message: 'Patriotic education deleted successfully',
      deletedId: deletedItem.id 
    });
  } catch (error) {
    console.error('Error deleting patriotic-education:', error);
    return NextResponse.json(
      { error: 'Failed to delete patriotic-education' },
      { status: 500 }
    );
  }
}
