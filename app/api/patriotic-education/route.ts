...
export async function GET() {
  try {
    console.log('🔍 GET /api/patriotic-education - початок виконання');
    
    const items = await prisma.patrioticEducation.findMany({
    ...
    console.error('❌ Error fetching patriotic-education:', error);
    return NextResponse.json(
      { error: 'Failed to fetch patriotic-education', details: error instanceof Error ? error.message : 'Unknown error' },
      { status: 500 }
    );
  }
}
...
export async function POST(request: NextRequest) {
  try {
    console.log('📝 POST /api/patriotic-education - початок виконання');
    ...
    const item = await prisma.patrioticEducation.create({
      data: {
        heading: heading || '',
    ...
    console.error('❌ Error creating patriotic-education:', error);
    console.error('❌ Stack trace:', error instanceof Error ? error.stack : 'No stack trace');
    
    // Повертаємо детальну помилку
    const errorResponse = {
      error: 'Failed to create patriotic-education',
