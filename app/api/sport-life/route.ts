...
export async function GET() {
  try {
    console.log('🔍 GET /api/sport-life - початок виконання');
    
    const sportLife = await prisma.sportLife.findMany({
      orderBy: { createdAt: 'desc' },
    });

    console.log('📊 Отримано записів з БД:', sportLife.length);

    // Конвертуємо JSON рядок назад в масив для кожного запису
    const sportLifeWithParsedPhotos = sportLife.map(item => {
      try {
        return {
          ...item,
    ...
    console.log('✅ Повертаємо записи:', sportLifeWithParsedPhotos.length);
    
    // Додаємо CORS заголовки
    const response = NextResponse.json(sportLifeWithParsedPhotos);
    ...
    const errorResponse = NextResponse.json(
      { error: 'Failed to fetch sport life', details: error instanceof Error ? error.message : 'Unknown error' },
      { status: 500 }
    );
...
export async function POST(request: NextRequest) {
  try {
    console.log('📝 POST /api/sport-life - початок виконання');
    ...
    const sportLife = await prisma.sportLife.create({ data });

    console.log('✅ Запис створено:', sportLife.id);
    ...
    console.error('❌ Error creating sport life:', error);
    
    const errorResponse = NextResponse.json(
      { error: 'Failed to create sport life', details: error instanceof Error ? error.message : 'Unknown error' },
      { status: 500 }
    );
