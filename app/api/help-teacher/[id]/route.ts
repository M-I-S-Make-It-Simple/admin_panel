...
export async function PUT(
  ...
  try {
    ...
    console.log('PUT request - data:', { title, content, text, link, linkText, titleEn, contentEn, textEn, linkTextEn });
    ...
    const updatedItem = await prisma.helpTeacher.update({
    ...
    console.error('Error updating help teacher item:', error);
    return NextResponse.json(
      { error: 'Failed to update help teacher item' },
      { status: 500 }
    );
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    await prisma.helpTeacher.delete({
      where: { id: parseInt(awaitedParams.id) },
    });

    return NextResponse.json({ message: 'Item deleted successfully' });
  } catch (error) {
    console.error('Error deleting help teacher item:', error);
    return NextResponse.json(
      { error: 'Failed to delete help teacher item' },
      { status: 500 }
    );
  }
}
