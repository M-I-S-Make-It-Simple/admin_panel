...
export async function GET() {
  try {
    console.log('🔍 GET /api/project-research - початок виконання');
    
    const items = await prisma.projectResearch.findMany({
    ...
    return NextResponse.json(
      { error: 'Failed to fetch project-research', details: error instanceof Error ? error.message : 'Unknown error' },
      { status: 500 }
    );
  }
}
...
export async function POST(request: NextRequest) {
  try {
    console.log('📝 POST /api/project-research - початок виконання');
    ...
    const item = await prisma.projectResearch.create({
      data: {
        heading: heading || '',
    ...
    const errorResponse = {
      error: 'Failed to create project-research',
