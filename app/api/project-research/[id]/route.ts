...
export async function PUT(
  ...
  try {
    ...
    const updatedItem = await prisma.projectResearch.update({
      where: { id },
      data: {
        heading: heading || '',
    ...
    console.error('Error updating project-research:', error);
    return NextResponse.json(
      { error: 'Failed to update project-research' },
      { status: 500 }
    );
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    const deletedItem = await prisma.projectResearch.delete({
      where: { id },
    });

    return NextResponse.json({ 
      message: 'Project research deleted successfully',
      deletedId: deletedItem.id 
    });
  } catch (error) {
    console.error('Error deleting project-research:', error);
    return NextResponse.json(
      { error: 'Failed to delete project-research' },
      { status: 500 }
    );
  }
}
