...
export async function GET() {
  try {
    console.log('🔍 GET /api/intellect-talent - початок виконання');
    
    const items = await prisma.intellectTalent.findMany({
    ...
    console.error('❌ Error fetching intellect-talent:', error);
    return NextResponse.json(
      { error: 'Failed to fetch intellect-talent', details: error instanceof Error ? error.message : 'Unknown error' },
      { status: 500 }
    );
  }
}
...
export async function POST(request: NextRequest) {
  try {
    console.log('📝 POST /api/intellect-talent - початок виконання');
    ...
    const item = await prisma.intellectTalent.create({
      data: {
        heading: heading || '',
    ...
    console.error('❌ Error creating intellect-talent:', error);
    console.error('❌ Stack trace:', error instanceof Error ? error.stack : 'No stack trace');
    
    // Повертаємо детальну помилку
    const errorResponse = {
      error: 'Failed to create intellect-talent',
