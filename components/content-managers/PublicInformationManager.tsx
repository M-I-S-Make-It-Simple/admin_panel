...
export default function PublicInformationManager() {
  const apiEndpoint = "/api/public-information";

  const [items, setItems] = useState<PublicInfoItem[]>([]);
  const [isEditing, setIsEditing] = useState(false);
  const [editingItem, setEditingItem] = useState<PublicInfoItem | null>(null);
  ...
  const handleEdit = (item: PublicInfoItem) => {
    setIsEditing(true);
    setEditingItem(item);
    setFormData({
      title: item.title || "",
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold">Публічна інформація</h1>
          <p className="text-sm text-gray-600 mt-1">Всього записів: {items.length}</p>
        </div>
        <Button className="bg-blue-500 hover:bg-blue-600" onClick={() => { setIsEditing(true); setEditingItem(null); resetForm(); setIsEditing(true); }}>
          Додати новий запис
        </Button>
      </div>
      ...
      {isEditing && (
        <Card>
          <CardHeader><CardTitle>{editingItem ? "Редагувати" : "Додати новий"}</CardTitle></CardHeader>
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                <Label htmlFor="title">Заголовок</Label>
                <Input id="title" value={formData.title} onChange={e => setFormData(p => ({ ...p, title: e.target.value }))} placeholder="Введіть заголовок" />
              </div>

              <div>
                <Label htmlFor="titleEn">Заголовок англійською</Label>
                <Input id="titleEn" value={formData.titleEn} onChange={e => setFormData(p => ({ ...p, titleEn: e.target.value }))} placeholder="Введіть заголовок англійською" />
              </div>

              <div>
                <Label htmlFor="content">Текст</Label>
                <Textarea id="content" rows={4} value={formData.content} onChange={e => setFormData(p => ({ ...p, content: e.target.value }))} placeholder="Введіть текст" />
              ...
              <div>
                ...
                <Textarea id="contentEn" rows={4} value={formData.contentEn} onChange={e => setFormData(p => ({ ...p, contentEn: e.target.value }))} placeholder="Введіть текст англійською" />
                <p className="text-xs text-gray-500 mt-1">Використовуйте Enter для нових рядків</p>
              </div>

              <div>
                <Label htmlFor="linkText">Текст посилання</Label>
                <Input id="linkText" value={formData.linkText} onChange={e => setFormData(p => ({ ...p, linkText: e.target.value }))} placeholder="Напр.: Публічний звіт (клікабельний текст)" />
              ...
              <div>
                ...
                <Input id="linkTextEn" value={formData.linkTextEn} onChange={e => setFormData(p => ({ ...p, linkTextEn: e.target.value }))} placeholder="Наприклад: Public Report (clickable text)" />
                <p className="text-xs text-gray-500 mt-1">Англійський варіант тексту посилання.</p>
              </div>

              <div>
                <Label htmlFor="url">Посилання (URL)</Label>
                <Input id="url" value={formData.url} onChange={e => setFormData(p => ({ ...p, url: e.target.value }))} placeholder="https://example.com/інформація" />
                    ...
                    <div className="flex flex-wrap gap-2">
                      {formData.photoUrls.map((u, i) => (
                        <div key={i} className="relative">
                          ...
                          <Button type="button" size="sm" variant="outline" className="absolute -top-2 -right-2 h-6 w-6 p-0 text-red-500 hover:text-red-700" onClick={() => removeImage(i)}>×</Button>
                        </div>
                      ))}
                    </div>
                  </div>
              ...
              <div className="flex flex-col h-full">
                <div className="flex-1">
                  {item.photoUrls && JSON.parse(item.photoUrls).length > 0 && (
                    <img src={JSON.parse(item.photoUrls)[0]} alt="Public info" className="w-full h-32 object-cover rounded mb-4" />
                  )}
