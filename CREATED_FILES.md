...
- `AccordionManager.tsx` - Універсальний компонент для управління статтяами
- `NewsManager.tsx` - Універсальний компонент для управління новинами з фото галереєю
- `SimpleContentManager.tsx` - Універсальний компонент для простих сторінок з текстом та фото
- `LinksManager.tsx` - Універсальний компонент для сторінок з посиланнями
- `MethodologicalEventsManager.tsx` - Компонент для управління методичними заходами
- `EvaluationCriteriaManager.tsx` - Компонент для управління критеріями оцінювання
- `ImageUploader.tsx` - Оновлений компонент для завантаження зображень (Cloudinary + локальний fallback)
- `sidebar.tsx` - Оновлений сайдбар з усіма 23 розділами
...
## 📁 Скрипти
- `scripts/generate-pages.js` - Скрипт для автоматичної генерації сторінок

## 🔄 Оновлені файли
- `components/sidebar.tsx` - Додано всі 23 розділи з логічним групуванням
- `prisma/schema.prisma` - Додано всі необхідні моделі для 23 розділів
- `app/(admin)/layout.tsx` - Видалено імпорт UploadThing стилів
- `components/ImageUploader.tsx` - Оновлено для роботи з Cloudinary + локальний fallback
